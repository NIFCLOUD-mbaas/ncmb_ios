...
@interface NCMBTwitterLoginView()

@property (retain, nonatomic) UIView *cv; //clear view
@property (retain, nonatomic) UIView *uv; //ui view

@end

@implementation NCMBTwitterLoginView
...
- (void)appearWebView:(UIInterfaceOrientation)interfaceOrientation{
    ...
    UIView* bg = [[UIView alloc] initWithFrame:(CGRect){0,0,self.webView.frame.size}];
    ...
    [self.uv addSubview:self.webView];
...
- (void)sizingWebView:(UIInterfaceOrientation)interfaceOrientation {
    ...
    switch (interfaceOrientation) {
        case UIInterfaceOrientationPortrait:{
            self.uv.transform = CGAffineTransformMakeRotation(0);
            ...
            self.webView.frame = CGRectMake(DEFAULT_MARGIN_WIDTH, DEFAULT_MARGIN_HEIGHT, self.uv.bounds.size.width - DEFAULT_MARGIN_WIDTH * 2, self.uv.bounds.size.height - (DEFAULT_MARGIN_HEIGHT * 2) - CLOSE_IMAGE_FRAME_SIZE);
            break;
        }case UIInterfaceOrientationLandscapeLeft:{
            self.uv.transform = CGAffineTransformMakeRotation(-M_PI/2.0);
            ...
            self.webView.frame = CGRectMake(DEFAULT_MARGIN_HEIGHT, DEFAULT_MARGIN_WIDTH, self.uv.bounds.size.width - DEFAULT_MARGIN_HEIGHT * 2, self.uv.bounds.size.height - (DEFAULT_MARGIN_WIDTH * 2) - CLOSE_IMAGE_FRAME_SIZE);
            break;
            ...
            self.uv.transform = CGAffineTransformMakeRotation(M_PI);
    ...
    bg.frame = CGRectMake(0, 0, self.webView.frame.size.width, self.webView.frame.size.height);
    UIView *iv = [self.webView viewWithTag:ActivityIndicatorTag];
    iv.center = CGPointMake(bg.frame.size.width/2, bg.frame.size.height/2);
    self.closeButton.frame = CGRectMake(self.uv.bounds.size.width/2 - CLOSE_BUTTON_WIDTH/2, DEFAULT_MARGIN_WIDTH * 1.7 + self.webView.bounds.size.height , CLOSE_BUTTON_WIDTH, CLOSE_BUTTON_HEIGHT);
}
